    ...
    <div class="mt-7 bg-white border border-slate-200 rounded-xl shadow-sm">
        <div class="p-4 sm:p-7">
            <div class="text-center">
                <h1 class="block text-2xl font-bold text-slate-800">{{ __('Register') }}</h1>
                <p class="mt-2 text-sm text-slate-600">
                    Already have an account?
                    <a class="text-indigo-600 hover:text-indigo-700 decoration-2 hover:underline font-medium transition-all" href="{{ route('login') }}">
                        Sign in here
                    </a>
                </p>
            </div>
            ...
            <div class="mt-5">
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <div class="grid gap-y-4">
                        <!-- Name -->
                        <div>
                            <label for="name" class="block text-sm mb-2 font-medium text-slate-700 font-semibold">{{ __('Name') }}</label>
                            <div class="relative">
                                <input type="text" id="name" name="name" value="{{ old('name') }}" class="py-3 px-4 block w-full border-slate-200 rounded-lg text-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:opacity-50 disabled:pointer-events-none @error('name') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror" required autocomplete="name" autofocus>
                                @error('name')
