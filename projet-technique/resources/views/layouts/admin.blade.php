...
<body class="bg-gray-50 antialiased">
    ...
    <div id="application-sidebar" class="hs-overlay hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed top-0 start-0 bottom-0 z-[60] w-64 bg-white border-e border-gray-200 pt-7 pb-10 overflow-y-auto lg:block lg:translate-x-0 lg:end-auto lg:bottom-0 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300">
        <div class="px-6">
            <a class="flex-none text-xl font-semibold text-indigo-600" href="{{ route('admin.players') }}" aria-label="Brand">Admin Panel</a>
        </div>
        <nav class="hs-accordion-group p-6 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
            <ul class="space-y-1.5">
                <li>
                    <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-slate-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('admin.players*') ? 'bg-gray-100 text-slate-900 font-semibold' : '' }}" href="{{ route('admin.players') }}">
                        <i data-lucide="graduation-cap" class="w-4 h-4"></i>
                        Students
                    </a>
                </li>
                <li>
                    <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-slate-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('admin.teams*') ? 'bg-gray-100 text-slate-900 font-semibold' : '' }}" href="{{ route('admin.teams.index') }}">
                        <i data-lucide="flag" class="w-4 h-4"></i>
                        Teams
                    </a>
                </li>
                <li class="pt-6 mt-6 border-t border-gray-200">
                    <a class="flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-slate-700 rounded-lg hover:bg-gray-100" href="{{ route('public.players') }}">
        ...
        <main>
            @if(session('success'))
                <div class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md mb-6" role="alert">
                    <div class="flex">
                        <div class="py-1"><i data-lucide="check-circle" class="w-6 h-6 mr-4 text-teal-500"></i></div>
                        <div>
